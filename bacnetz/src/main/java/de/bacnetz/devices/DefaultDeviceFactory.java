...
public class DefaultDeviceFactory implements Factory<Device> {
    ...
    private void addPropertiesToDoorCloseStateDevice(final Device device) {
        ...
        deviceProperty = new DefaultDeviceProperty<byte[]>("present-value", DeviceProperty.PRESENT_VALUE,
                new byte[] { (byte) (((Boolean) device.getPresentValue()) ? 1 : 0) }, MessageType.ENUMERATED);
        device.getProperties().put(deviceProperty.getPropertyKey(), deviceProperty);
...
//                  new Date());
        deviceProperty = new DefaultDeviceProperty<Integer>("time-of-device-restart",
                DeviceProperty.TIME_OF_DEVICE_RESTART, 1, MessageType.UNSIGNED_INTEGER);
        device.getProperties().put(deviceProperty.getPropertyKey(), deviceProperty);
