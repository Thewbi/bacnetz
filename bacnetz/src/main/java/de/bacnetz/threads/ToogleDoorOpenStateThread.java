...
import de.bacnet.services.CommunicationService;
...
public class ToogleDoorOpenStateThread implements Runnable {
    ...
    public void run() {
        ...
        while (true) {
            ...
            parentObjectIdentifierServiceParameter.setObjectType(ObjectIdentifierServiceParameter.OBJECT_TYPE_DEVICE);
            parentObjectIdentifierServiceParameter.setInstanceNumber(NetworkUtils.DEVICE_INSTANCE_NUMBER);
            ...
            childObjectIdentifierServiceParameter
                    .setObjectType(ObjectIdentifierServiceParameter.OBJECT_TYPE_BINARY_INPUT);
            childObjectIdentifierServiceParameter.setInstanceNumber(1);

            final Device childDevice = device.findDevice(childObjectIdentifierServiceParameter);
            ...
            apdu.setInvokeId(device.retrieveNextInvokeId());
            ...
            presentValueServiceParameter.setPayload(new byte[] { (byte) value });
            ...
            try {
                final InetAddress datagramPacketAddress = InetAddress.getByName("192.168.2.2");
